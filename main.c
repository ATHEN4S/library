...
int main() {
  ...
  if (quest_cadastro == 'n') {
    ...
    } while (autorizar == 0 || autorizar2 != 0 || strlen(Usuario.user) > 20);
    ...
    while (result == 1) {
      ...
      case 'a':
        ...
        if (aval(Usuario, &Usuario) == 2) {
          ...
          printf(
              "\n Você já tem um livro! Para pegar outro, devolva-o primeiro. "
              "Pode conferir qual livro é e até quando tem para entrega-lo na "
              "opção C, 'Conferir data de devolução' do menu.");
        break;
...
int checagem_usuario(const tUsuario login, const tUsuario *pLogin,
  ...
  if (!check_user) {
    ...
    while (fgets(buffer, 100, check_user)) {
      ...
      if (strcmp(pLogin->user, value) ==
          0) { // Se o input corresponder ao que o stktok ta rodando...
      ...
      if (col == 4) {
        if (marcador == 1) {
          ...
          if (strcmp(livroprocurado, "/DISPONIVEL") >= 10) {
            ...
            printf("Esse livro já foi emprestado. Escolha outro, ou tente "
    ...
    while (procurando != NULL) {
      ...
      if (col == 2 && marca == 1) {
        if (strlen(procurando) > 13) {
          ...
          printf("\n Você ainda não pegou um livro! Para pegar, selecione a "
                 "opção A, 'Pegar um livro.' no menu do usuário.");
          marca = 0;
          return 0;
        }
      }
...
int mudarIND(char *frase,
              int line) { // MUDAR PARA INDISPONIVEL NO LIVROS, PEGAR EMPRESTADO

  FILE *fp;
  FILE *fv;
  int count = 0;                  // Contar o numero das linhas no txt
  int edited = 0;                 // como bool, 0=Falso, 1=Verdadeiro
  char *mudar = ",/INDISPONIVEL"; // String que queremos adicionar no final do
                                  // livro selecionado.
...
int adiciona_date(int dia, int mes, int ano, tUsuario *pUser,
                  tLivro *plivro) // MUDAR PARA INDISPONIVEL NO LEITORES TXT,
                                  // PEGAR EMPRESTADO, BOTAR DATA DE ENTREGA
{
  FILE *data_r;
  FILE *temp;
